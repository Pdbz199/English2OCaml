    ...
    | If(s) -> let memStr = mem_from_str s in
      final_string t (
        acc ^ "if" ^ (if String.length memStr > 0 then " " ^ memStr
                        ...
                        (replace_keys 
                           [" is equal to"; " is"; " equals";] " =" s)) ^ "\n")
    | Then(s) -> final_string t (acc ^ "then " ^ (
        if String.length s = 0 then "" else
        if List.mem_assoc (String.trim s) !environment then s
        else "\"" ^ (String.trim s) ^ "\"") ^ "\n")
    | Else(s) -> final_string t (acc ^ "else " ^ (
        if String.length s = 0 then "" else
        if List.mem_assoc (String.trim s) !environment then s
        else "\"" ^ (String.trim s) ^ "\"") ^ "\n")
    | Let(s) ->
      let str = "let" ^ (replace_keys [","] ";"
                           (replace_keys 
                              ["= = = ="; "= = ="; "= =";] "=" (
                              replace_keys 
                                ["to"; "be"; "equal"] "=" s))) ^ " in\n" in 
      let alteredStrLst = (String.split_on_char '=' 
                             (String.trim (replace_keys ["let "] "" str))) in
      let varname = (String.trim (List.hd alteredStrLst)) in
      let varval = (List.nth (String.split_on_char ' '
                                (List.nth alteredStrLst 1)) 1) in
      ignore(
      ...
      let strng = if is_num varval || is_bool varval || is_lst varval
        then str else 
          let memStr = mem_from_str str in
          if String.length memStr = 0
          then replace_keys [varval] ("\"" ^ varval ^ "\"") str 
          else "let " ^ varname ^ " = " ^ memStr in
      final_string t (acc ^ strng)
    | Print(s) -> let formatted = remove_keys [", "] (String.trim s) in
      let strng = 
        let memStr = mem_from_str formatted in
        if String.length memStr > 0
        then "print_string (string_of_bool (" ^ memStr ^ "))"
        else
          let input = (
            match List.assoc_opt formatted !environment with
            | None -> ""
            | Some(s) -> s) in
          if is_lst input then let toCheck = (remove_keys ["[";";";"]"] input)
            in if is_num toCheck
            then "List.iter (Printf.printf \"%d \") " ^ formatted
            else "List.iter (Printf.printf \"%s \") " ^ formatted
          else if is_num s then "print_int " ^ formatted
          else if is_bool formatted
          then "print_string (string_of_bool " ^ formatted ^ ")"
          else (
            if List.mem_assoc formatted !environment 
            then 
              let varname = formatted in
              let varval = List.assoc varname !environment in
              if is_num varval then "print_int " ^ varname else 
              if is_bool varval then
                ("print_string (string_of_bool " ^ varname ^ ")")
              else "print_string " ^ varname
            else if is_lst formatted then let newForm = 
                                            replace_keys [","] ";" formatted in
              if is_num (remove_keys ["[";";";"]"] newForm)
              then "List.iter (Printf.printf \"%d \") " ^ newForm
              else "List.iter (Printf.printf \"%s \") " ^ newForm
            else "print_string \"" ^ formatted ^ "\"") in
      final_string t (acc ^ strng ^ "\n")
